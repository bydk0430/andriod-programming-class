...
public class VoiceRecorder implements OnCompletionListener, OnErrorListener {
    ...
    // 인식불가능한 SD 카드이면 일시적 저장소 사용
    public static final String TEMP_STORAGE = "/data/data/com.example.student.multimemo/voice/";
    ...
    // 리셋 레코더 상태. 샘플이 레코딩 됐을떄, 파일이 디스크에서 사라졌으면 새로운 레코딩으로 사용
    public void clear() {
        stop();
    ...
    public void startRecording(int outputfileformat, String extension, Context context) {
        ...
        // 레코딩이 시작못하면 RuntimeException 핸들
        try {
            mRecorder.start();
